...
curl --fail --show-error --location --progress-bar \
    "https://gh.llkk.cc/ollama-linux-${ARCH}.tgz${VER_PARAM}" | \
...
if [ -f /etc/nv_tegra_release ] ; then
    if grep R36 /etc/nv_tegra_release > /dev/null ; then
        ...
        curl --fail --show-error --location --progress-bar \
            "https://gh.llkk.cc/ollama-linux-${ARCH}-jetpack6.tgz${VER_PARAM}" | \
            $SUDO tar -xzf - -C "$OLLAMA_INSTALL_DIR"
    elif grep R35 /etc/nv_tegra_release > /dev/null ; then
        status "Downloading JetPack 5 components"
        curl --fail --show-error --location --progress-bar \
            "https://gh.llkk.cc/ollama-linux-${ARCH}-jetpack5.tgz${VER_PARAM}" | \
            $SUDO tar -xzf - -C "$OLLAMA_INSTALL_DIR"
    else
...
if check_gpu lspci amdgpu || check_gpu lshw amdgpu; then
    ...
    curl --fail --show-error --location --progress-bar \
        "https://gh.llkk.cc/ollama-linux-${ARCH}-rocm.tgz${VER_PARAM}" | \
        $SUDO tar -xzf - -C "$OLLAMA_INSTALL_DIR"
