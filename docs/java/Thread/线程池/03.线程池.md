...
线程工厂 ThreadFactory 主要用来创建线程，可以通过 ThreadFactory 来定制线程的属性，比如线程名、线程优先级、是否守护线程等。JVM 中默认使用 DefaultThreadFactory 来创建线程。
...
`execute(Runnable command)` 主要负责提交任务，任务提交到线程池之后，线程池会根据当前线程池的状态，以及线程池的参数，来决定如何处理任务。
...
添加线程的方法是自旋的方式，通过CAS的方式添加线程，添加成功直接跳出循环，失败则重新获取ctl的值，然后进行自旋。

自旋的方式添加线程，是为了防止多线程并发添加线程时，线程数超过maximumPoolSize，导致添加失败。
...
以下是之前写这个文章的时候拿的代码，忘记是哪个版本了，可以简单看一下。

> 后面写文章还是得声明是基于什么版本写的，不然很容易误导人啊
